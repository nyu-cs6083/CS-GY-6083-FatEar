...
const newReviewsByFriends = async (user) => {
  try {
    const reviewByfriends =  await db.getDBObject().query(`SELECT * FROM reviewSong NATURAL JOIN song ` + 
                                                          `WHERE username IN ( SELECT user2 FROM friend WHERE acceptStatus = 'Accepted' AND user1 = "${user}" UNION SELECT user1 FROM friend ` +
